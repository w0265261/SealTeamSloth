...
for [{_x=0},{_x<=_count},{_x=_x+10}] do {
	...
	_query = format["SELECT houses.id, houses.pid, houses.pos, players.name, houses.security, houses.leasers, houses.listPrice FROM houses INNER JOIN players ON houses.pid=players.playerid WHERE houses.owned='1' LIMIT %1,10",_x];
	...
	
	{
		...
		_house setVariable["security",_x select 4, true];
		_leasers = _x select 5;
		_house setVariable["leasers",_leasers, true];
		if((_x select 6) > 0) then {_house setVariable["listPrice",(_x select 6),true]};
