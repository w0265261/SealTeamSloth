...
*/
private["_itemList","_index","_config","_priceTag","_displayName"];
_index = param [1,-1,[0]];
...
switch (_index) do
{
	case 0:
	{
		...
		_config = [_shop] call life_fnc_weaponShopCfg;
		{
			...
			_displayName = if(isNil {_x select 1}) then {_itemInfo select 1} else {_x select 1};
			_itemList lbAdd format["%1", _displayName];
