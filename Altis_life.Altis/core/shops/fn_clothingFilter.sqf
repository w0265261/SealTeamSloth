...
private["_control","_selection","_list","_filter","_displayName","_index"];
...
_clothes = switch (life_clothing_store) do
{
	...
	case "cop": {
		if(undercover) then {[_selection] call life_fnc_clothing_bruce} 
		else {[_selection] call life_fnc_clothing_cop}
	};
	...
	case "bh": {[_selection] call life_fnc_clothing_c3;};
};
...
if(count _clothes == 0) exitWith {};
{
	...
	{
		_displayName = (getText(configFile >> (_details select 6) >> (_x select 0) >> "DisplayName"));
		_index = _list lbAdd format["%1", _displayName];
		_list lbSetTooltip [_index, format["%1", _displayName]];
	}
		else
	{
		_index = _list lbAdd format["%1", _x select 1];
		_list lbSetTooltip [_index, format["%1", _x select 1]];
	};
