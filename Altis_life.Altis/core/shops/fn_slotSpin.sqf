...
_betamt = param [0,1,[0]];
if(life_handpaper < _betamt) exitWith {hint format["You don't have enough money to play (%1$)",_betamt];};
["cash","take",_betamt] call life_fnc_handlePaper;
...
switch (_slot) do
{
	...
	case 211:{_winnings = 8;};
	case 123:{_winnings = 80;};
	case 234:{_winnings = 85;};
	...
	case 333:{_winnings = 200;};
	case 444:{_winnings = 200;};
	case 555:{_winnings = 150;};
	case 666:{_winnings = 4000;};
	case 777:{_winnings = 10000;};
	default {_winnings = 0;}
};

/*multiple winnings by bet amount*/
_slotcash = _winnings * ( _betamt / 100 );

if (_slotcash > 250000) then {
    _slotcash = 250000;
};

["cash","add",_slotcash] call life_fnc_handlePaper;
_winningsText ctrlSetText format["You won $%1!",_slotcash];
