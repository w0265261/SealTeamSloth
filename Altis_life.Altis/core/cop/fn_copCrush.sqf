...
if(!(_veh in["I_G_Offroad_01_armed_F","O_MRAP_02_F","I_MRAP_03_F"] || _vehicle getVariable["stolen",false]) && call life_coplevel < 5) exitWith {hint "You may not crush this type of vehicle."; };
...
if((_vehicle isKindOf "Car") || (_vehicle isKindOf "Air") || (_vehicle isKindOf "Ship")) then
{
	...
	if(player distance _vehicle > 10) exitWith {hint "Crushing Cancelled"; life_action_inUse = false;};
	...
	if((count crew _vehicle) == 0) then
	{
		...
		switch (true) do
		{
			...
			case (_vehicle isKindOf "Ship"): {_price = 1000};
			case (_vehicle isKindOf "Air"): {_price = 2000};
		};
		
		life_impound_inuse = true;
		[_vehicle] remoteExec ["TON_fnc_vehicleIsDead",2];
		_vehicle spawn
		{
			sleep 1;
