...
createDialog "STS_life_ticket_pay";
...
[] spawn
{
	...
	if(isNull (findDisplay 2600) && !life_ticket_paid && !life_poorBoy) then
	{
		[0,format[localize "STR_Cop_Ticket_Refuse",profileName]] remoteExec ["life_fnc_broadcast",west];
		[1,format[localize "STR_Cop_Ticket_Refuse",profileName]] remoteExec ["life_fnc_broadcast",life_ticket_cop,false];
	};
};