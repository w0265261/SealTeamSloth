	...
	else
{
	switch (_animate) do
	{
		...
		case "cop_offroad":
		{
			if(typeOf _vehicle == "C_Offroad_01_F" && _color != 7) exitWith {}; //This is an undercover car, do not animate!
			if(typeOf _vehicle in ["C_Offroad_01_F","B_G_Offroad_01_armed_F"]) then {
				waitUntil {!isNil {_vehicle getVariable "Life_VEH_color"}};
			};
			_vehicle animate ["HidePolice", 0];
