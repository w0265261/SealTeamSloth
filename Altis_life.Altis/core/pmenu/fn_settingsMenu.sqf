...
{
	if(!createDialog "STS_SettingsMenu") exitWith {hint "Something went wrong, the menu won't open?"};
};
...
_env = _display displayCtrl 2972;
_grass = (findDisplay 2900) displayCtrl 2973;
...
if(life_environ) then {
	_env ctrlSetTextColor [0,1,0,1];
	_env ctrlSetText "ON";
} else {
	_env ctrlSetTextColor [1,0,0,1];
	_env ctrlSetText "OFF";
};

if(!isNil "myGrassCheck") then {_grass ctrlSetText 'ON'; _grass ctrlSetTextColor [0,1,0,1]} else {_grass ctrlSetText 'OFF'; _grass ctrlSetTextColor [1,0,0,1]};