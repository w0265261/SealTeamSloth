...
/*Near players*/
_near_units = [];
{ if((player distance _x < 10) && !(_x getVariable ["invis",false])) then {_near_units pushBack _x;};} foreach playableUnits;
{
	if(!isNull _x && alive _x && player distance _x < 10 && _x != player) then
	{
		_name = "";
		if((headGear _x in _headGear || goggles _x in _goggles) && !(_x in units player)) then {_name = "Masked Man"} else {_name = _x getVariable["realname",name _x]};
		_near lbAdd format["%1 - %2",_name,(if(_x getVariable["undercover",false]) then {"CIV"} else {side _x})];
		_near lbSetData [(lbSize _near)-1,str(_x)];
		_near_i lbAdd format["%1 - %2",_name,(if(_x getVariable["undercover",false]) then {"CIV"} else {side _x})];
		_near_i lbSetData [(lbSize _near)-1,str(_x)];
	};
} foreach _near_units;

_mstatus ctrlSetStructuredText parseText format["<img size='1.3' image='icons\bank.paa'/> <t size='0.8px'>$%1</t><br/><img size='1.2' image='icons\money.paa'/> <t size='0.8'>$%2</t>",[life_myfunds] call life_fnc_numberText,[life_handpaper] call life_fnc_numberText];
