...
for "_i" from 0 to (count _units)-1 do
{
	switch (true) do
	{
		...
		{
			_members lbAdd format["%1 [Leader]",(_units select _i) getVariable["realname",name (_units select _i)]];
		};
		
		case ((getPlayerUID (_units select _i)) == _ownerID) :
		{
			_members lbAdd format["%1 [Founder]",(_units select _i) getVariable["realname",name (_units select _i)]];
		};
		
		default 
		{
			_members lbAdd format["%1",(_units select _i) getVariable["realname",name (_units select _i)]];
		};
	};
