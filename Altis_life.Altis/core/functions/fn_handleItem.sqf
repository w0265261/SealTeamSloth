...
*/
private["_item","_details","_bool","_ispack","_items","_isgun","_ongun","_override","_toUniform","_toVest","_sync"];
_item = param [0,"",[""]];
...
_sync = param [7,false,[false]]; /*sync or not*/
		...
		case "CfgWeapons":
		{
				...
				else
			{
				switch((_details select 5)) do
				{
					...
					case 801: {if(uniform player == _item && !(_item in backpackItems player)) then {removeUniform player} else {player removeItem _item};};
					case 701: {if(vest player == _item && !(_item in backpackItems player)) then {removeVest player} else {player removeItem _item};};
...
if(_sync) then {
	if(!life_diagIsWaiting) then {
		life_diagIsWaiting = true;
		[] spawn {
			waitUntil{!dialog};
